...
//  4-Wheel Soccer Bot  |  FlySky FS-i6  |  BTS7960 x2
//
//  Two motors per side wired in PARALLEL to each BTS7960.
...
//    5V          →  Arduino 5V
//
//  Wiring — Left BTS7960 (drives Left Motor A + B in parallel)
//    RPWM → D5    LPWM → D6
//    R_EN → D7    L_EN → D8
//    B+   → battery +   (direct)
//    M+   → Left Motor A (+) and Left Motor B (+) joined
//    M-   → Left Motor A (-) and Left Motor B (-) joined
//
//  Wiring — Right BTS7960 (drives Right Motor A + B in parallel)
//    RPWM → D9    LPWM → D10
//    R_EN → D11   L_EN → D12
//    B+   → battery +   (direct)
//    M+   → Right Motor A (+) and Right Motor B (+) joined
//    M-   → Right Motor A (-) and Right Motor B (-) joined
//
//  Wiring — Power
...
//
//  NOTE: Two parallel motors double the current per driver.
//  Use 16–18 AWG wire between BTS7960 and motors.
//  Add heatsinks to both BTS7960 boards.
// ============================================================
...
// ── Left BTS7960 (Left Motor A + B in parallel) ──────────────
#define L_RPWM   5
#define L_LPWM   6
#define L_R_EN   7
#define L_L_EN   8

// ── Right BTS7960 (Right Motor A + B in parallel) ────────────
#define R_RPWM   9
#define R_LPWM   10
#define R_R_EN   11
#define R_L_EN   12
...
// ── driveMotor ───────────────────────────────────────────────
// Drives one BTS7960 H-bridge (and both parallel motors on it).
...
void loop() {
  ...
  // Tank mixing:
  //   Left  wheels (Motor A + B) = throttle + steering
  //   Right wheels (Motor A + B) = throttle − steering
